...
const backfillChatContexts = async () => {
  const summary = {
    seededContexts: 0,
    mergedContexts: 0,
    chatsUpdated: 0,
    ...
    `
      WITH base AS (
        SELECT
          ...
          ch.context_id,
          CASE
            WHEN ch.context_type = 'request' THEN ch.context_id
            ELSE ch.request_id
          END AS request_id,
          CASE
            WHEN ch.context_type = 'booking' THEN ch.context_id
            ELSE ch.booking_id
          END AS booking_id,
          ch.response_id,
      ...
      SELECT
        ...
        booking_id,
        response_id,
        COALESCE(request_created_at, booking_created_at, chat_created_at),
        COALESCE(
          request_updated_at,
          booking_updated_at,
          chat_updated_at,
  ...
  summary.seededContexts = seedResult.rowCount ?? 0
    ...
    `
      WITH ranked AS (
        SELECT
          ...
          master_id,
          FIRST_VALUE(id) OVER (
            PARTITION BY client_id, master_id
      ...
      SELECT
        ...
        cc.updated_at
      FROM ranked
      JOIN chat_contexts cc ON cc.chat_id = ranked.id
      WHERE ranked.canonical_id <> cc.chat_id
      ON CONFLICT (chat_id, context_type, context_id)
      DO UPDATE SET
        request_id = COALESCE(EXCLUDED.request_id, chat_contexts.request_id),
        booking_id = COALESCE(EXCLUDED.booking_id, chat_contexts.booking_id),
  ...
  summary.mergedContexts = mergeResult.rowCount ?? 0
    ...
    `
      WITH ranked AS (
        SELECT
          ...
          cc.response_id,
          ROW_NUMBER() OVER (
            PARTITION BY cc.chat_id
            ORDER BY cc.updated_at DESC NULLS LAST, cc.created_at DESC, cc.id DESC
          ) AS rn
        FROM chat_contexts cc
      )
      UPDATE chats ch
      SET context_type = ranked.context_type,
          ...
          request_id = CASE
            WHEN ranked.context_type = 'request'
              THEN COALESCE(ranked.request_id, ranked.context_id)
            ELSE NULL
          END,
          booking_id = CASE
            WHEN ranked.context_type = 'booking'
              THEN COALESCE(ranked.booking_id, ranked.context_id)
            ELSE NULL
          END,
          response_id = CASE
            WHEN ranked.context_type = 'request'
              THEN COALESCE(ranked.response_id, ch.response_id)
            ELSE NULL
          END,
          updated_at = NOW()
      FROM ranked
      WHERE ranked.rn = 1
        AND ch.id = ranked.chat_id
        AND ch.context_type <> 'support'
    `
  )
  summary.chatsUpdated = updateResult.rowCount ?? 0
...
const start = async () => {
  ...
  const normalizedChatBackfill = normalizeText(process.env.CHAT_CONTEXTS_BACKFILL)
  const shouldBackfillChatContexts =
    normalizedChatBackfill === '1' || normalizedChatBackfill.toLowerCase() === 'true'

  await ensureSchema()
  await seedLocations()
  await fs.mkdir(uploadsRoot, { recursive: true })

  let shouldAutoBackfillChatContexts = false
  if (!shouldBackfillChatContexts) {
    try {
      const contextCountResult = await pool.query(
        `SELECT COUNT(*)::int AS count FROM chat_contexts`
      )
      const contextCount = Number(contextCountResult.rows[0]?.count ?? 0)
      if (contextCount === 0) {
        ...
        const chatCount = Number(chatCountResult.rows[0]?.count ?? 0)
        shouldAutoBackfillChatContexts = chatCount > 0
      }
    } catch (error) {
      console.error('Chat context backfill check failed:', error)
    }
  }

  if (shouldBackfillChatContexts || shouldAutoBackfillChatContexts) {
    try {
      console.log('Running chat context backfill...')
      const summary = await backfillChatContexts()
      console.log('Chat context backfill complete:', summary)
    } catch (error) {
      console.error('Chat context backfill failed:', error)
    }
  }
